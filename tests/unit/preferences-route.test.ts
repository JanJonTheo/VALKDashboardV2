...
function put(body: unknown) {
  return PUT(new Request("http://localhost/api/preferences/test", {
    method: "PUT",
...
describe("preference storage limit", () => {
  ...
  it("forwards collections using schema version 3", async () => {
    const current = { filters: {}, sorting: [], visibleColumns: [], pageSize: 25 };
  ...
  it("also counts UTF-8 bytes and the upstream envelope", async () => {
    const body = {
      filters: { values: Array.from({ length: 31 }, () => "é".repeat(256)), tail: "" },
      sorting: [], visibleColumns: [], pageSize: 25,
    };
    const bytes = new TextEncoder().encode(JSON.stringify(body)).length;
    body.filters.tail = "x".repeat(16384 - bytes - 10);
    expect(new TextEncoder().encode(JSON.stringify(body)).length).toBeLessThan(16384);
